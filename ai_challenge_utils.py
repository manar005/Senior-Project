...
def normalize_submitted_flag(user_input: str) -> str:
    if user_input is None:
        return ""
    s = str(user_input).strip().replace("\r", "").replace("\n", "")
    for c in "\ufeff\u200b\u200c\u200d\u200e\u200f":
        s = s.replace(c, "")
    return s


def flags_match(stored_flag_field: str, user_input: str) -> bool:
    """Compare stored FLAG = \"x\" with user submission (value only)."""
    inner = extract_flag_inner_value(stored_flag_field)
    if inner is None:
        return False
    submitted = normalize_submitted_flag(user_input)
    return submitted == inner
