...
def insert_challenges(conn, challenge_data_list):
    """Insert challenges. Each item: (category_id, title, description, hint, flag, expected_outcome, challenge_type, order_in_category, points)"""
    conn.executemany('''
        INSERT INTO challenges (category_id, title, description, hint, flag, expected_outcome, challenge_type, order_in_category, points)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    ''', challenge_data_list)
    conn.commit()
