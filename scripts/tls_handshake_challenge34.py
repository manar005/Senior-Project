"""
Generate TLS 1.2 handshake traffic for Challenge 34 (TLS version identification).
...
1. Writes cert/key to static/keys/challenge_34_cert.pem and challenge_34_key.pem
   (optional: solvers only need the pcap).
2. Starts tshark or tcpdump on loopback to write static/pcaps/challenge_34.pcapng,
   runs a short TLS session, then stops the capture.

Usage (from project root):
  python3 scripts/tls_handshake_challenge34.py
...
KEY_FILE = os.path.join(KEY_DIR, "challenge_34_key.pem")
CERT_FILE = os.path.join(KEY_DIR, "challenge_34_cert.pem")
PCAP_PATH = os.path.join(PROJECT_ROOT, "static", "pcaps", "challenge_34.pcapng")
    ...
    if cmd:
        ...
        if sz > 200:
            ...
            print(
                ...
                "then run: python3 scripts/tls_handshake_challenge34.py "
                "(with no capture tools the script only runs TLS; save as challenge_34.pcapng).",
                file=sys.stderr,
            )
            sys.exit(1)
    else:
